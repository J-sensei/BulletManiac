...
namespace BulletManiac.Scenes
{
    public class MainMenuScene : Scene
    {
        ...
        {
            spriteBatch.DrawString(ResourcesManager.FindSpriteFont("Font_64"), TITLE, titlePosition, Color.White);
            spriteBatch.DrawString(ResourcesManager.FindSpriteFont("Font_22"), VERSION, versionPosition, Color.White);
            spriteBatch.DrawString(ResourcesManager.FindSpriteFont("Font_22"), FOOTER, footerPosition, Color.White);
        ...
        public override void Initialize()
        {
            ...
            // Title
            float offsetX = ResourcesManager.FindSpriteFont("Font_64").MeasureString(TITLE).X / 2;
            titlePosition = new Vector2(screenSize.Width / 2 - offsetX, screenSize.Height / 2 - 300f);

            // Version 
            float x = ResourcesManager.FindSpriteFont("Font_22").MeasureString(VERSION).X / 2;
            versionPosition = new Vector2(screenSize.Width / 2 + offsetX - x * 2, screenSize.Height / 2 - 220f);

            // Footer
            x = ResourcesManager.FindSpriteFont("Font_22").MeasureString(FOOTER).X / 2;
            footerPosition = new Vector2(screenSize.Width / 2 - x, screenSize.Height / 2 + 310f);
            // Buttons
            Button tutorialBtn = new Button(new Vector2(screenSize.Width / 2, screenSize.Height / 2), new Vector2(2.8f, 2.5f), "Tutorial");
            Button playBtn = new Button(new Vector2(screenSize.Width / 2, screenSize.Height / 2 + 70f), new Vector2(2.8f, 2.5f), "Play");
            Button settingBtn = new Button(new Vector2(screenSize.Width / 2, screenSize.Height / 2 + 140f), new Vector2(2.8f, 2.5f), "Setting");
            Button quitBtn = new Button(new Vector2(screenSize.Width / 2, screenSize.Height / 2 + 210f), new Vector2(2.8f, 2.5f), "Quit");
